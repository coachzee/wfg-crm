...
describe('Gmail OTP Service', () => {
  it('should have MyWBH email credentials configured', () => {
  ...
  it('should connect to MyWBH Gmail account via IMAP', async () => {
    const credentials = getMyWFGCredentials();
    const result = await verifyGmailCredentials(credentials);
    
    expect(result.success).toBe(true);
    if (!result.success) {
      console.error('MyWBH Gmail connection failed:', result.error);
    }
